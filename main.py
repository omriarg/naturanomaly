...
def init_system(video_path):
    ...
    csv_filename = f'tracked_objects.csv'
...
def run_main_routine_loop(video_path):

    # initialize the csv_filename, results, bbox_path, fame_counter variables
    csv_filename, results, bbox_path, fame_counter = init_system(video_path)
    ...
    for result in results:
        ...
        if fame_counter == 1:
            ...
            out = cv2.VideoWriter('object_detection.mp4', fourcc, fps, (width, height))
        ...
        cv2.imshow('routine map', routine_map_display)
...
def run_main_anomaly_loop(video_name_routine=None):
    csv_filename, results, bbox_path, frame_counter = init_system(video_name_routine)
    ...
    for result in results:
        ...
        if frame_counter == 1:
            ...
            height = int(height)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter('anomaly_detection.mp4', fourcc, fps, (width, height))
        # Detect objects
        bboxes, labels, confidences = detect_objects(result)
        ...
        save_tracked_objects_to_csv(tracked_objects, csv_filename=None)
...
if __name__ == "__main__":
    ...
    download_missing_video_files(video_name_routine=video_name_routine,
                                 video_link='https://drive.google.com/uc?id=1X6QN3wLnglkqTOL-bbVVWvq1RKsTIXY0')
    run_main_routine_loop(video_name_routine)
    video_name_test = os.path.join(current_direcotry, 'test1.mp4')
    download_missing_video_files(video_name_routine=video_name_test,
